...
import { asyncTotal, completed, DefaultR, IO, pure, raiseAbort, raiseError, runToPromiseExit, suspended, unit } from "../src/io";
import * as io from "../src/io";


export function expectExitIn<E, A, B>(ioa: IO<DefaultR, E, A>, f: FunctionN<[Exit<E, A>], B>, expected: B): Promise<void> {
...
export function expectExit<E, A>(ioa: IO<DefaultR, E, A>, expected: Exit<E, A>): Promise<void> {
    return expectExitIn(ioa, identity, expected);
}

export const assertEq = <A>(S: Eq<A>) => (a1: A) => (a2: A): IO<DefaultR, never, void> =>
    S.equals(a1, a2) ? unit : raiseAbort(`${a1} <> ${a2}`);

export function eqvIO<E, A>(io1: IO<DefaultR, E, A>, io2: IO<DefaultR, E, A>): Promise<boolean> {
...
export function exitType<E, A>(io1: IO<DefaultR, E, A>, tag: Exit<E, A>["_tag"]): Promise<void> {
    return runToPromiseExit(io1)
...
export function arbIO<E, A>(arb: Arbitrary<A>): Arbitrary<IO<DefaultR, E, A>> {
    return arbVariant
        .chain((ioStep) => {
            if (ioStep === "succeed") {
                return arb.map((a) => pure(a) as IO<DefaultR, E, A>); // force downcast
...
export function arbConstIO<E, A>(a: A): Arbitrary<IO<DefaultR, E, A>> {
 ...
 * @param arb
 */
export function arbKleisliIO<E, A, B>(arbAB: Arbitrary<FunctionN<[A], B>>): Arbitrary<FunctionN<[A], IO<DefaultR, E, B>>> {
...
Arbitrary<FunctionN<[E], IO<DefaultR, E2, A>>> {
 ...
 * @param arbE
 */
export function arbErrorIO<E, A>(arbE: Arbitrary<E>): Arbitrary<IO<DefaultR, E, A>> {
    return arbE
        .chain((err) =>
 ...
 * @param e
 */
export function arbConstErrorIO<E, A>(e: E): Arbitrary<IO<DefaultR, E, A>> {
    return arbErrorIO(fc.constant(e));
}

export function arbEitherIO<E, A>(arbe: Arbitrary<E>, arba: Arbitrary<A>): Arbitrary<IO<DefaultR, E, A>> {
    return fc.boolean()
